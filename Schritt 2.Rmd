...
library(ggplot2)
colnames(df_1)
model <- lm(cardio ~ BMI+ap_hi+ap_lo+cholesterol+gluc+smoke+alco+active+BMI, data=df_1  )
summary(model)
sigma(model)/mean(df_1$cardio)
ggplotRegression(lm(cardio ~ BMI, data=df_1))
ggplot(df_1, aes(x = BMI, y = cardio)) + 
  geom_point() +
